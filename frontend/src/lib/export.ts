  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        ...
        // Footer
        new Paragraph({
          children: [new TextRun({ text: 'Generated by BrightClause', italics: true, size: 20 })],
...
export async function exportToPDF(
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    pdf.text('Generated by BrightClause', 105, 285, { align: 'center' })
...
export async function exportReport(report: ReportData): Promise<void> {
  ...
  pdf.setFontSize(24)
  pdf.setFont('helvetica', 'bold')
  pdf.text('EXECUTIVE SUMMARY REPORT', 105, yPos, { align: 'center' })
  yPos += 10

  pdf.setFontSize(11)
  pdf.setFont('helvetica', 'normal')
  pdf.setTextColor(100, 100, 100)
  pdf.text(`${report.document_info.filename} | ${formatDate()}`, 105, yPos, { align: 'center' })
  pdf.setTextColor(0, 0, 0)
  yPos += 15
  ...
  pdf.setFontSize(10)
  pdf.setFont('helvetica', 'normal')
  const summaryLines = pdf.splitTextToSize(report.executive_summary, 170)
  ...
  autoTable(pdf, {
    ...
    head: [['Risk Level', 'Count']],
    body: [
      ['Critical', String(report.risk_overview.critical)],
      ['High', String(report.risk_overview.high)],
      ['Medium', String(report.risk_overview.medium)],
      ['Low', String(report.risk_overview.low)],
    ],
    theme: 'grid',
    headStyles: { fillColor: [201, 162, 39] },
  ...
  pdf.setFontSize(14)
  pdf.setFont('helvetica', 'bold')
  pdf.text('Key Clauses', 20, yPos)
  yPos += 8

  autoTable(pdf, {
    startY: yPos,
    head: [['Clause', 'Risk', 'Summary']],
    body: report.key_clauses.map(c => [
      c.clause_type,
      c.risk_level?.toUpperCase() ?? 'N/A',
      c.summary.substring(0, 100) + (c.summary.length > 100 ? '...' : ''),
    ]),
    theme: 'grid',
    headStyles: { fillColor: [201, 162, 39] },
  ...
  report.recommendations.forEach((rec, i) => {
    ...
    const lines = pdf.splitTextToSize(`${i + 1}. ${rec}`, 170)
  ...
  pdf.save(`${report.document_info.filename.replace('.pdf', '')}_executive_report.pdf`)
}
